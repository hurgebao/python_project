...
if len(stocksDB):
	...
	delSq1="delete from t_jqdata_all_stocks where 1=1";
	#print(delSq1)
	try:
	   # 执行SQL语句
	   cur.execute(delSq1)
	...
	# 关闭连接
	delConnect.close()
	print('\n 获取所有股票数据 --> 数据库：清除表数据 == 成功!! ===') 
else:
	print('\n 获取所有股票数据 --> 数据库：表数据为空,不需要清除 == 成功!! ===') 
